...
function Dpass() {
  ...
  const clickPlusOrReduce = throttle(
    function (e: any) {
      const { id } = e.target;
      if (id === 'plus') {
  ...
  const setImg = throttle(
    function (ind: number) {
      setPage(1);
      setIsHistory(false);
      setDPassHistory([]);
      const index = passList.findIndex(
        (res: any) => res?.name === imgSta?.name
      );
      if (index > -1) {
        const at = index + ind;
        if (at > 3) {
  ...
  const paramsAll: any = {
    ...
    point: {
      ...
      '4': '480',
      '3': '1200',
      '1': Number(imgSta?.stopAt)
        ? dayjs.unix(Number(imgSta?.stopAt)).format('YYYY-MM-DD HH:mm:ss')
        : t('Dpass.mi'),
    },
    title: {
      '2': t('Dpass.Creation'),
  ...
  const show = (back: string) => {
    if (imgSta?.passId) {
      ...
      if (at === '1') {
        ...
        (at === '2' && Number(user?.rewardPointCnt) > 6000) ||
        (at === '4' && Number(user?.rewardPointCnt) > 480) ||
        (at === '3' && Number(user?.rewardPointCnt) > 1200)
      ) {
        ...
        <div className="dpass-background">
          <div className="dpass-content">
            ...
            <div className="dpass-content-right">
              <p className="dpass-content-right-title">
                {' '}
