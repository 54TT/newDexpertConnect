...
function task({ getParams, data, select, setSelect, params }: any) {
  ...
  const [option, setOption] = useState(
    activityOptions ? activityOptions : params[0]
  );
  ...
  const getRank = async () => {
    ...
    const res = await getAll({
      method: 'get',
      url: '/api/v1/campaign/point/rank',
      data: {},
      token: token,
    });
    if (res?.status === 200) {
  ...
  const list: any = [
    { name: 'special', value: t('Active.Special') },
    { name: 'd', value: 'D Pass' },
    { name: 'ranking', value: t('Active.Ranking') },
    { name: 'first', value: t('Active.First') },
    { name: 'daily', value: t('Active.Daily') },
  ];
  const now =
    list.filter((item: any) => params.indexOf(item.name) !== -1) || [];
  return (
    <>
      <div
        className={'activeOptions disCen'}
        style={{
          width: browser ? '72%' : '92%',
          margin: browser ? '35px auto 42px' : '15px auto',
        }}
      >
