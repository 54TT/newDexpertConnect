...
function TwitterRelease({
  ...
  return (
    <div className={'twitterTask'}>
      ...
      <p className={'link disCen'} onClick={openLink}>
        <img src="/x.svg" alt="" /> <span> {t('Active.Tweet')}</span>
      </p>
      <img
        src="/twitterCase.png"
        alt=""
      ...
      <div className={'bot disDis'}>
        <p onClick={handleCancel} className="p">
          {t('Active.Cancel')}
        </p>
        <div
          onClick={() => {
          ...
          className="p"
        >
          {t('Active.Confirm')} {isConfirm && <Load />}
        </div>
      </div>
    </div>
  );
