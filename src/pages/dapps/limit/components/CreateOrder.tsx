...
export default function CreateOrder() {
  ...
  const [payTokenBalance, setPayTokenBalance] = useState<Decimal>(
    new Decimal(0)
  );
  const [receiveTokenBalance, setReceiveTokenBalance] = useState<Decimal>(
    new Decimal(0)
  );
  ...
  const approveOder = async () => {
    ...
    if (payTokenAmount && receiveTokenAmount) {
      const payTokenAmountInWei = ethers.utils.parseUnits(
        payTokenAmount,
        payToken.decimals
      );
      const receiveTokenAmountInwei = ethers.utils.parseUnits(
        Number(receiveTokenAmount)
          .toFixed(Number(receiveToken.decimals))
          .toString(),
  ...
  const getToeknUnitPrice = async (
    ...
    try {
      ...
      if (type === 'receive')
        setReceiveTokenUnitPrice(Number(amount.toFixed(6)));
    } catch (error) {
      return null
    }
    // 展示只截取6位
  ...
  const getAmount = async (
    ...
    if (
      ((type === 'pay' || type === 'receive') && !payToken?.contractAddress) ||
      !receiveToken?.contractAddress
    ) {
      return;
    }
  ...
  // 获取汇率
  const getTokenRateDebounce = useCallback(debounce(getExchangeRate, 2000), [
    payToken,
    receiveToken,
    provider,
  ]);
  ...
  useEffect(() => {
    if (Number(payTokenAmount) <= Number(payTokenBalance)) {
      if (Number(tokenRate) !== 0 && payTokenAmount) {
        setRateLoading(false);
        setReceiveTokenAmount(
          (Number(payTokenAmount) * Number(tokenRate)).toString()
        );
      }
    } else {
    ...
    <div className="createOrder">
      <div className="wrapper">
        <div className="pay token-card">
          <div className="token-card-header dis-between">
            <span className="token-card-header-left">{t('limit.pay')}</span>
            {isLogin ? (
          ...
          <div className="token-card-body dis-between">
            ...
            <Input
              ...
              onChange={(e) => {
                if (Number(e.target.value) <= Number(payTokenBalance))
                  setPayTokenAmount(e.target.value);
                if (Number(e.target.value) > Number(payTokenBalance))
                  setPayTokenAmount(payTokenBalance.toString());
          ...
          <div className="token-card-footer dis-between">
            <span className="token-card-footer-left">{payToken?.symbol}</span>
            <span className="token-card-footer-right">
              {isLogin ? (
                <>
                  ~$
                  {Number(payTokenAmount) * payTokenUnitPrice === 0
                    ? 0
                    : (Number(payTokenAmount) * payTokenUnitPrice)?.toFixed(6)}
                </>
              ) : (
                <></>
              )}
        ...
        <div className="receive token-card">
          <div className="token-card-header dis-between">
            <span className="token-card-header-left">{t('limit.receive')}</span>
            {isLogin ? (
          ...
          <div className="token-card-footer dis-between">
            ...
            <span className="token-card-footer-right">
              {isLogin ? (
                <>
                  ~$
                  {Number(receiveTokenAmount) * receiveTokenUnitPrice === 0
                    ? 0
                    : (
                        Number(receiveTokenAmount) * receiveTokenUnitPrice
                      )?.toFixed(6)}
                </>
              ) : (
                <></>
      ...
      <div className="wrapper choose-card">
        <div className="choose-card-left dis-col">
          <span>
            {t('limit.pay')}{' '}
            {isExchangeRate ? receiveToken?.symbol : payToken?.symbol}{' '}
            {t('limit.at rate')}
          </span>
          {rateLoading ? (
            <Skeleton.Button active size="small" />
        ...
        <div className="choose-card-middle dis-col">
          <span className="market-price" onClick={() => getTokenRateDebounce()}>
            {t('limit.market')}
          </span>
          <div
          ...
          >
            <span>
              {isExchangeRate ? payToken?.symbol : receiveToken?.symbol}
            </span>
      ...
      <div className="unit-price">
        <div className="unit-price-row">
          <div
            ...
            onClick={() => {
              setIsShowUnitPrice(!isShowUnitPrice);
            }}
          >
            <span>
              {receiveToken?.symbol} {t('limit.price')}
            </span>
            <img
              style={{
                width: '14px',
                transform: `rotate(${isShowUnitPrice ? '180deg' : '0deg'})`,
              }}
              src="/arrowDown.svg"
            />
          </div>
          <div className="unit-price-row-right">
            ...
            {rateLoading ? (
              ...
              <span style={{ color: '#86f097' }}>
                {numberToFixed((1 / Number(tokenRate)) * payTokenUnitPrice)}
              </span>
            )}
        ...
        <div
          ...
          style={{ opacity: isShowUnitPrice ? '1' : '0' }}
        >
          <div className="">
            <span>
              {payToken?.symbol} {t('limit.price')}
            </span>
          </div>
          <div>
            {rateLoading ? (
            ...
            {rateLoading ? (
              ...
              <span style={{ color: '#86f097' }}>
                {numberToFixed(Number(tokenRate) * receiveTokenUnitPrice)}
              </span>
            )}
          </div>
        </div>
