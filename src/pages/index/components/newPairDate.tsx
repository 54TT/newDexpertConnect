    ...
    <>
      {tableDta.map((record: any, ind: number) => {
        ...
        return b && b.includes('T') && b.length > 10 ? (
          ...
          <div
            key={ind}
            className={`indexNewPairBodyData dis ${browser ? '' : 'indexNewPairBodyDataSmall'}`}
            onClick={() => push(record)}
          >
            <div className={`indexTableLogo `}>
              <img
                loading={'lazy'}
                src={record?.collect ? '/collectSelect.svg' : '/collect.svg'}
                style={{ display: 'none' }}
                alt=""
              ...
              <div>
                ...
                >
                  {simplify(record?.token0?.symbol?.replace(/^\s*|\s*$/g, ''))}
                </p>
                <div
                  style={{
                    fontSize: '14px',
                ...
                >
                  {simplify(record?.token1?.symbol?.replace(/^\s*|\s*$/g, ''))}
                </div>
              </div>
            </div>
            <div style={{ color: 'white' }}>
              {Number(record?.priceUSD) ? setMany(record?.priceUSD) : 0}
            </div>
            <div
              style={{
                color:
              ...
              {[
                ...
                return (
                  ...
                  key={index}
                    title={
                      i.name === 'eth' ? (
                        <p className='eth'> <span>{t('Market.eth')}</span> <span>{record?.token0?.id}</span> </p>
                      ) : i.name === 'uncx' ? (
                        t('Market.bluur')
                      ) : (
                        t('Market.uni2')
