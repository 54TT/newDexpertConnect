...
function Center() {
  ...
  const video = <>
    {
      params?.id === 'create' ? <p className={'pp'} style={{ marginBottom: '20px' }}>
        {router.pathname === "/app"
          ? t("Dapps.Thor Desc")
          : t("Dapps.Run Tips")}
      </p> : <>
        <p className={'pp'} style={{ marginBottom: '6px' }}>{t("Dapps.a")}</p>
        <p className={'pp'} style={{ marginBottom: '6px' }}>{t("Dapps.b")}</p>
        <p className={'pp'}>{t("Dapps.c")}</p>
      </>
    }
    <p style={{ color: "rgb(130,230,150)", margin: '5px 0' }}><span>{t("Dapps.fee")}</span><span style={{ marginLeft: "10px" }}>{params?.id === 'create' ? '0.08eth' : 'Swap 0.2%, Sniper 0.5%'}</span></p>
